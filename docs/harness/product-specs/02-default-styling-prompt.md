...
```json
{
  ...
  "title": "Change Default Visual Style Properties",
  "description": "Interactively change the default values of node and edge visual properties (colors, sizes, shapes, fonts, etc.) for the current network's visual style.",
  "arguments": []
}
```
...
```json
{
  ...
  "description": "Set the default value of a single visual property in the active visual style. The value format depends on the property type: hex color string for Paint properties, numeric for Double/Integer, display name string for shapes and line types, 'Family-Style-Size' for fonts.",
  "inputSchema": {
    "type": "object",
    "properties": {
      "property_id": {
