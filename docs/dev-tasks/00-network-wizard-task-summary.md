...
│   │   └── requires: VisualPropertyUtils (shared utilities)
│   └── set_visual_default tool
│       └── requires: VisualPropertyUtils
...
│   │   └── requires: VisualPropertyUtils (parseValue)
...
| 6 | **Refactor `load_cytoscape_network_view` — add `source` dispatch and NDEx adapter** | Modify `LoadNetworkViewTool` input schema: add `source` (enum: ndex/file), rename `network-id` to `network_id`, add `file_path`/`source_column`/`target_column`/`interaction_column` fields. Add `source` dispatch in handler. Adapt existing NDEx loading to the `source=ndex` branch (logic stays same, just gated on source). Update tool description. Return JSON response `{status, network_suid, node_count, edge_count, network_name}` instead of plain text. Update existing tests for new schema. | 01 §4.1, 01 §5.1, 01 §5.1.1 |
| 7 | **`load_cytoscape_network_view` — native file format support** | Add `handleFileImport()` for native formats (.sif, .gml, .xgmml, .cx, .cx2, .graphml, .sbml, .owl, .biopax) using `LoadNetworkFileTaskFactory`. File existence validation, timeout handling (120s), error responses. Tests: mock `LoadNetworkFileTaskFactory`, verify file loading flow, file-not-found error. | 01 §5.1.2 |
| 8 | **Implement `get_file_columns` tool** | New tool class that reads column headers + sample rows from CSV/TSV files (delimiter detection by extension) and Excel files (.xls/.xlsx) via Apache POI (`WorkbookFactory.create(file)`). Tests: test CSV, TSV, and Excel parsing with mock/temp file content. | 01 §4.2, 01 §5.2 |
| 9 | **`load_cytoscape_network_view` — tabular file import with column mapping** | Add `handleTabularImport()` for CSV/TSV/Excel files requiring source/target column mapping. Uses Cytoscape's command API (`CommandExecutorTaskFactory`) or equivalent task factory for table-based network import. Tests: mock command execution, verify column mapping arguments are passed correctly. | 01 §5.1.3 |
...
3. Ask agent: *"Load the network from file /tmp/test.sif"* → agent calls `load_cytoscape_network_view` with `source=file, file_path=/tmp/test.sif` → network appears in Cytoscape
...
- **Apache POI for Excel**: Task 1 adds `poi` + `poi-ooxml` as `embed` dependencies; Task 8 implements full CSV/TSV/Excel column reading.
