 ...
 *
 * <p>Uses {@link PipedInputStream}/{@link PipedOutputStream} to feed messages to the server, and a
 * {@link LineCountingOutputStream} to capture responses with deterministic synchronization (no
 * {@code Thread.sleep}).
 *
 * <p>Usage:
 *
...
public class InMemoryTransport {
    ...
    /**
     * Builds and starts the MCP server with the given tools pre-registered. Tools are registered
     * via the builder so no {@code tools/list_changed} notification is emitted.
     *
     * <p>The server's reader thread is started by {@code build()}. Messages written to the feeder
     * are buffered in the pipe and read when the thread is ready — no startup sleep needed.
     */
    ...
    /**
     * Sends a single JSON-RPC message to the server. Non-blocking — the message is written to the
     * pipe buffer and processed by the server's reader thread.
     */
     ...
     *
     * <p>In the MCP protocol, each JSON-RPC request produces one response line; notifications
     * produce none. The standard init + tool-call flow produces 2 response lines (init response +
     * tool call result).
     */
    ...
    /**
     * Blocks until the server has written at least {@code expectedResponses} newline-terminated
     * lines to its output stream.
     *
    ...
    /** Returns everything the server wrote as UTF-8. Each JSON-RPC response is a separate line. */
    ...
    /**
     * OutputStream that counts newline-terminated lines and supports blocking until a target line
     * count is reached via {@code wait/notify}.
     */
